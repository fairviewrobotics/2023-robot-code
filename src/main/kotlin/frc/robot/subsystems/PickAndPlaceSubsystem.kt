...
class PickAndPlaceSubsystem(elevatorMotorId : Int,
    ...
    object Telemetry {
        ...
        val wristVoltageNT = NetworkTableInstance.getDefault().getTable("Arm").getDoubleTopic("WristVoltage").publish()
        val elbowVoltageNT = NetworkTableInstance.getDefault().getTable("Arm").getDoubleTopic("ElbowVolts").publish()
        val elevatorVoltageNT = NetworkTableInstance.getDefault().getTable("Arm").getDoubleTopic("ElevatorVolts").publish()
        val intakeVoltageNT = NetworkTableInstance.getDefault().getTable("Arm").getDoubleTopic("IntakeVolts").publish()
    ...
    init {
        ...
        elbowEncoder.positionConversionFactor = 2.0 * Math.PI / 60.0
     ...
     */
    val absoluteWristPosition get() = 2.0
    ...
    override fun periodic() {
        ...
        Telemetry.elbowPosition.set(elbowEncoder.position)
        Telemetry.elevatorPosition.set(elevatorEncoder.position)
        Telemetry.wristPosition.set(wristEncoder.position)
        Telemetry.elbowVelocity.set(elbowEncoder.velocity)
        Telemetry.topHit.set(topHit)
        Telemetry.bottomHit.set(bottomHit)
        Telemetry.wristVoltageNT.set(wristVoltage)
        Telemetry.elbowVoltageNT.set(elbowVoltage)
        Telemetry.elevatorVoltageNT.set(elevatorVoltage)
        Telemetry.intakeVoltageNT.set(intakesVoltage) //petition to call intake "intakes"
