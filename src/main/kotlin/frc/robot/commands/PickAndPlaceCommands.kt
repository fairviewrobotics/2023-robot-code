...
class SetPickAndPlacePosition(val continuous: Boolean ,val subsystem: PickAndPlaceSubsystem,
    ...
    override fun initialize() {
        ...
        wristPid.setTolerance(0.07)//might need changing
...
fun Base(pnp: PickAndPlaceSubsystem): Command {
    return SequentialCommandGroup(
        SetPickAndPlacePosition(
            ...
            { if (!CommandValues.cube) 1.6 else 0.0 }, //in
            { if (!CommandValues.cube) -1.8 else 0.0 } //in
        ),
        SetPickAndPlacePosition(
            true,
            pnp,
            ...
            { if (!CommandValues.cube) 1.6 else 0.0 }, //in 1.0
            { if (!CommandValues.cube) -1.8 else 0.0 } //in -1.2
        )
    )
}
