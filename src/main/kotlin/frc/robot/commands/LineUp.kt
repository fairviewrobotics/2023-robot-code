...
fun LineUp(driveSubsystem: SwerveSubsystem, limelight: LimelightSubsystem): Command {
    val targetpose = limelight.getTargetPose()

    if (targetpose.contentEquals(DoubleArray(0))) {
        return SequentialCommandGroup()
    }

    val trajectory: PathPlannerTrajectory
    val driveUtils = DriveUtils(driveSubsystem)
